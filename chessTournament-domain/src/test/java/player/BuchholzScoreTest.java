...
class BuchholzScoreTest {
    ...
    void testSonnebornBergerScore() throws InvalidRatingException, InvalidRatingNumberException, InvalidListNumberException, InvalidPlayerInfoException, InvalidPlayerNameExeption, InvalidResultException {
        ...
        Player playerA = builder().playerInfo("player", "A", "Club A", 1).rating(rating).build();
        Player playerB = builder().playerInfo("player", "B", "Club A", 2).rating(rating).build();
        Player playerC = builder().playerInfo("player", "C", "Club A", 3).rating(rating).build();
        Player playerD = builder().playerInfo("player", "D", "Club A", 4).rating(rating).build();
        Player playerE = builder().playerInfo("player", "E", "Club A", 5).rating(rating).build();
        Player playerF = builder().playerInfo("player", "F", "Club A", 6).rating(rating).build();
        Player playerG = builder().playerInfo("player", "G", "Club A", 7).rating(rating).build();
